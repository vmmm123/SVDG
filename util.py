...
def validate(test_loader,net,device):
    ...
    for step, batch in enumerate(test_loader):
        ...
        _, predicted = torch.max(net(images=val_x.to(device)), 1)
...
class ModelCheckpoint():
    ...
    def save_model(self, model, current_value, test_acc,epoch,version,target):
        if self.monitor_op(current_value, self.best):
            ...
            torch.save(model.state_dict(), os.path.join(self.directory1,'{}_{}_best_epoch.pth'.format(version,target)))
